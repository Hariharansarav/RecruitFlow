...
export default function EvaluationDetailModal({ isOpen, evaluation, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full p-6 sm:p-8 space-y-6 text-slate-900 overflow-y-auto max-h-[90vh]">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 rounded-2xl bg-zinc-50 border border-zinc-200/80 text-xs">
          <div className="space-y-1">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 block">
              Position
            </span>
            <div className="flex items-center gap-1.5 font-semibold text-zinc-800 text-sm">
          ...
          <div className="space-y-1 sm:border-l sm:border-zinc-200/80 sm:pl-4">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 block">
              Evaluation Details
            </span>
            <div className="flex items-center gap-1.5 text-zinc-700 font-medium">
              ...
              <span>
                ...
                <strong className="text-zinc-900">
                  {evaluation.tech_lead?.name || evaluation.hr?.name || 'Recruiter'}
                </strong>
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {/* Overall Score */}
          <div className={`p-4 rounded-2xl border text-center space-y-1 ${getScoreColor(scoreNum)}`}>
            <span className="text-[11px] font-bold uppercase tracking-wider block opacity-80">
              Overall Score
            </span>
            <div className="flex items-center justify-center gap-1">
              <Star className="w-5 h-5 fill-current" />
              <span className="text-2xl font-black">{scoreNum.toFixed(2)}</span>
              <span className="text-xs font-semibold opacity-70">/ 5.0</span>
            </div>
          </div>

          {/* JD Match Percentage */}
          <div className={`p-4 rounded-2xl border text-center space-y-1 ${getMatchColor(matchNum)}`}>
            <span className="text-[11px] font-bold uppercase tracking-wider block opacity-80">
              JD Competency Match
            </span>
            <div className="flex items-center justify-center gap-1">
              <Award className="w-5 h-5 text-current" />
              <span className="text-2xl font-black">{Math.round(matchNum)}%</span>
            </div>
          </div>

          {/* Total Points */}
          <div className="col-span-2 sm:col-span-1 p-4 rounded-2xl border border-zinc-200 bg-white text-center space-y-1">
            ...
            <div className="flex items-center justify-center gap-1 text-zinc-900">
              ...
              <span className="text-xs font-semibold text-zinc-400">
                / {evaluation.max_score ?? skills.length * 5} max
              </span>
            </div>
          </div>
        </div>
              ...
              {skills.map((skillItem) => {
                ...
                return (
                  <div
                    ...
                    className="p-3 rounded-2xl border border-slate-100 bg-slate-50/50 space-y-1.5"
                  >
        ...
        {evaluation.notes && (
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-2xl bg-zinc-50 border border-zinc-200/80 text-sm text-zinc-700 leading-relaxed italic">
            ...
            {resumeUrl ? (
              ...
              >
                <Button
                  variant="secondary"
                  size="sm"
                  className="flex items-center gap-2 border-indigo-200 text-indigo-700 hover:bg-indigo-50 hover:border-indigo-300 font-semibold"
                >
